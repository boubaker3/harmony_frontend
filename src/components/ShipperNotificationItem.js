...
export default function ShipperNotificationsItem(props) {
  ...
  const handleClose = (event) => {
    ...
    axios.post("http://localhost:8000/api/updateRequestStatus",{notif_id:props.item.chef_shipper_id,status:sentStatus})
    .then((response)=>{
    setOpen(false);
...
if(props.item.chef_shipper_id){
  ...
  setNotification(`your chef accepted ${props.item.quantity} ${props.item.product_name} from ${props.item.name} `)
}
...
<Avatar component={Link} to={{pathname:"/clientProfile",search:`?userid=${props.item.userid}`}} src={props.item.photo} sx={{borderRadius:"100%",width:{xs:"60px",md:" 70px"},height:{xs:"60px",md:" 70px"} }}> 
</Avatar>
</Grid>
<Grid xs={9} sx={{margin:' auto 0',textAlign:"left"}}> 
       <Typography noWrap  sx={{ fontFamily:"Montserrat", textOverflow:'ellipsis' }}>{notification}</Typography>
                  ...
                  <ClickAwayListener onClickAway={close}>
                    <MenuList
                      > 
                       ...
                       <MenuItem value="0" onClick={handleClose}>Refuse It </MenuItem>
