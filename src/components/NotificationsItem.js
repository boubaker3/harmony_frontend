...
export default function NotificationsItem(props) {
  ...
  const handleClickSnackBar = () => {
    axios.get(`http://localhost:8000/api/deleteOrders?orderid=${props.item.order_id}`)
    .then(response => {
...
<Grid xs={9} sx={{margin:' auto 0',textAlign:"left"}}> 
       <Typography noWrap  sx={{ fontFamily:"Montserrat", textOverflow:'ellipsis' }}>You ordred {props.item.product_name} from {props.item.name}</Typography>
