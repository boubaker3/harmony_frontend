...
export default function ChatRoom() {
  ...
  const sendMsg = (event) => {
    ...
    axiosInstance
      .post(`sendMsg?userid=${userid}`, { sender: user.userid, msg: sentMsg })
      .then((response) => {
        setSentMsg("");
      })
      .catch((error) => {});
  ...
  useEffect(() => {
    axiosInstance
      .get(`retrieveMsgs?sender=${user.userid}&receiver=${userid}`)
      .then((response) => {
      ...
      >
        ...
        >
          {msgs.map((msg) => (
            <Box
              key={msg.id}
              sx={{ display: "block", marginTop: "5px", width: "800px" }}
            >
              <Box sx={{ right: 0 }}>
                <Avatar
                  src={
                    user.userid == msg.sender_user_id
                      ? user.photo
                      : msg.sender_user_photo
                  }
                  sx={{
                    width: { xs: "50px" },
                    height: "50px",
                ...
                >
                  <Typography
                    sx={{
                      fontFamily: "Montserrat",
                      color:
                        msg.sender_user_id == user.userid ? "white" : "gray",
                    }}
                  >
                    {msg.msg}
                  </Typography>
